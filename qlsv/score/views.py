...
class ScoreViewSet(viewsets.ModelViewSet):
    ...
    def avg_by_student(self, request):
        ...
        expr_total = ExpressionWrapper((F("Midterm") * F("WeightMidterm")) + (F("Final") * F("WeightFinal")) + (F("Other") * F("WeightOther")), output_field=FloatField())
        qs = Score.objects.all()
        if scode:
            qs = qs.filter(StudentCode=scode)
        data = qs.values("StudentCode", "FullName").annotate(avg_total=Avg(expr_total), total_records=Count("pk")).order_by("StudentCode")
        res = [{"StudentCode": r["StudentCode"], "FullName": r["FullName"], "RecordsCount": r["total_records"], "AverageTotal": round(float(r["avg_total"] or 0), 2)} for r in data]
    ...
    def chart_data(self, request):
        ...
        for t in totals:
            ...
            bins[i] += 1
        return response.Response({"labels": [str(i) for i in range(11)], "counts": bins, "count": len(totals), "avg": round(sum(totals) / len(totals), 2)}, status=200)
    ...
    def manual_edit(self, request):
        ...
        score, _ = Score.objects.get_or_create(StudentCode=data["StudentCode"], CourseName=data["CourseName"], defaults={"FullName": data.get("FullName", "")})
        mapping = {"FullName": "FullName", "Midterm": "Midterm", "Final": "Final", "Other": "Other", "WeightMidterm": "WeightMidterm", "WeightFinal": "WeightFinal", "WeightOther": "WeightOther"}
...
def score_dashboard(request):
    ...
    stats = Score.objects.aggregate(avg_gpa=Avg(expr_total), max_score=Max(expr_total), min_score=Min(expr_total))
    context = {"total_students": total_students, "students_with_scores": total_records, "average_gpa": f"{(stats.get('avg_gpa') or 0):.2f}", "max_score": f"{(stats.get('max_score') or 0):.2f}" if stats.get("max_score") is not None else "N/A", "min_score": f"{(stats.get('min_score') or 0):.2f}" if stats.get("min_score") is not None else "N/A", "scores_list": Score.objects.order_by("-pk")[:50]}
